...
Uma única aba do Chromium, colunas coletadas em sequência.
"""
...
STAGGER_SECONDS  = int(os.environ.get("STAGGER_SECONDS", 5))
...
# Flags ultra-conservadoras de memória
LAUNCH_ARGS = [
    "--no-sandbox",
    "--disable-setuid-sandbox",
    "--disable-dev-shm-usage",
    "--disable-gpu",
    "--single-process",                    # um único processo — economiza ~100MB
    "--disable-extensions",
...
class BrowserManager:
    ...
    async def start(self):
        ...
        try:
            ...
            if "login" in self.page.url or "i/flow" in self.page.url:
                log.error("❌ Não autenticado — verifique X_COOKIES_JSON")
            else:
    ...
    async def _ensure_page(self) -> Page:
        """Garante que temos uma página funcional."""
        ok = False
        try:
            ok = (self.browser and self.browser.is_connected() and
                  self.page and not self.page.is_closed())
        except Exception:
            pass

        if not ok:
            log.warning("⚠️  Browser/página mortos — relançando...")
            await self._launch()

        return self.page

    async def fetch(self, url: str) -> list[dict]:
        """Navega para URL e extrai tweets. Thread-safe via lock."""
