...
REFRESH_INTERVAL = int(os.environ.get("REFRESH_INTERVAL", 60))   # segundos
STAGGER_SECONDS  = int(os.environ.get("STAGGER_SECONDS", 5))     # entre colunas
MAX_TWEETS       = int(os.environ.get("MAX_TWEETS", 30))
PAGE_WAIT        = float(os.environ.get("PAGE_WAIT", 6))
X_COOKIES_JSON   = os.environ.get("X_COOKIES_JSON", "")          # env var com cookies
# ─────────────────────────────────────────────────────────
...
def build_url(query: str, sort: str = "live") -> str:
    ...
    return (
        ...
        f"?q={urllib.parse.quote(compact)}"
        f"&f={sort}"
        f"&src=typed_query"
    )
    ...
    async def start(self):
        ...
        if X_COOKIES_JSON:
            try:
                ...
                log.error(f"❌ Erro ao injetar cookies: {e}")
        else:
            log.warning("⚠️  X_COOKIES_JSON não definido — buscas podem falhar sem login")
        ...
        try:
            ...
            if "login" in url or "i/flow" in url:
                log.error("❌ Não autenticado — verifique os cookies em X_COOKIES_JSON")
            else:
                log.info("✅ Autenticado no X")
        except Exception as e:
            log.warning(f"Verificação de auth falhou: {e}")
        finally:
            await page.close()
...
class XDeckApp:
    ...
    async def refresh_all(self):
        for i, col_id in enumerate(sorted(self.subscriptions)):
            if i > 0:
